...
from fastapi import APIRouter, Depends, Query, WebSocket, WebSocketDisconnect, HTTPException, Response
...
from constants import POSTGRES_DATABASE, POSTGRES_HOST, POSTGRES_PASSWORD, POSTGRES_USER, POSTGRES_SCHEMA, POSTGRES_PORT, SLM_MODEL_NAME, LLM_MODEL_NAME, EMBEDDING_MODEL_NAME
...
async def send_message(
    ...
    try:
        ...
        if response:
            ...
            raise HTTPException(status_code=500, detail="An error occured when generating a response")
...
async def chat_stream(
    ...
    try:
        ...
        if auth_data.get("type") == "auth" and "token" in auth_data:
            ...
            if not user_id:
                await websocket.close(code=4003, reason="Invalid or expired session token")
                return
        else:
